...
def compute_dist_pac_pellet(x1,y1,x2,y2,s_x,s_y, width) :
    dist = 0
    delta_x = 0
    delta_y = 0
    if (x1 == x2) :
...
#  x = x prec y = y prec et ordre MOVE
    ...
    for i in range(len(pacs_x)) :
        row = []
        for j in range(len(liste_10)):
            row.append(10000)
        matrice_d.append(row)
        
    # Gestion grosses pastilles
    # Affecter les packs restants avec les grosses pastilles
    # Calculer matrice  des distances (pac X big pellet)

    index_pac_mini = -1
    index_big_mini = -1
    d_mini = 1000 
    if  all_affected(pacs_affectation) and all_affected(liste_10) :
        for i in range(len(pacs_x)) :
            for j in range(len(liste_10)):
                n_pellet = int(liste_10[j])

                d = compute_dist_pac_pellet(pacs_x[i],pacs_y[i],liste_x[j],liste_y[j], width)
                if d <d_mini:
                    d_mini = d
                    index_big_mini = j
                    index_pac_mini = i
    ...
    n_pac = 0
    while not all_affected(pacs_affectation) :
        # On regarde s'il reste des  big pastilles à a affecter
        if not all_affected(big_affectations) :
            # recherche de la pastille la plus proche
            d_mini = 1000
            index_mini = 0
            for n in len(liste_10) :
                if not big_affectations[n]:
                    n_pellet = int(liste_10[n])
        ...
        #Recherche dans les pastilles visibles les plus proches
        elif not all_affected(small_affectation):
    ...
    #On s'occupe des grosses pastilles d'abord
    if len(big_affectations) 
