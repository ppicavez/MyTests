    ...
    if len(pacs_x_ennemy) > 0:
        ...
        for j in range(len(pacs_x)) :    
            for i in range(len(pacs_x_ennemy)) :
                d = abs(pacs_x[j] - pacs_x_ennemy[i]) 
                if d == width : #Repliemnt de l'axe x
                    d = 0
                d = d + abs(pacs_y[j] - pacs_y_ennemy[i])
                print("Mine : "+pacs_type_id[j], file=sys.stderr)
                print("Ennemy : "+pacs_type_id_ennemy[i], file=sys.stderr)
