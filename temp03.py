...
def find_adjacentes(x,y,sx,sy,width) :
    result = []
    result.append(index_square(x,y + 1,sx,sy,width))
    result.append(index_square(x + 1,y + 1,sx,sy,width))
    result.append(index_square(x + 1,y,sx,sy,width))
    result.append(index_square(x + 1,y - 1,sx,sy,width))
    result.append(index_square(x,y - 1,sx,sy,width))
    result.append(index_square(x - 1,y - 1,sx,sy,width))
    result.append(index_square(x -1 ,y,sx,sy,width))
...
# game loop
while True:
    print(" Debut du tour ", file=sys.stderr)
    ...
    print(" Fin décodage pacs ", file=sys.stderr)
...
#Création liste des pellets à 10 points
    ...
    print(" Nb pellets visibles " +str(visible_pellet_count), file=sys.stderr)
    ...
    print(" Fin de boucle ", file=sys.stderr)
    ...
    print(" Affectations pellets visibles aux  pacs ", file=sys.stderr)
    liste_adj = []
    ...
    for i_pellet_small in range(nb_small_pellet) :
        found = False    
        i_pac = 0
        i_pellet = liste_1[i_pellet_small]
        square_pellet = liste_no_square[i_pellet] 
        #print(" Pellet "+str(i_pellet), file=sys.stderr)
        #print(" Case Pellet "+str(square_pellet), file=sys.stderr)

        #recherche ds les cases adjacentes des pacs
        while not found and i_pac < nb_mine_pacs :
            print(" Passsage 1", file=sys.stderr)
            i_square = pacs_square_index[i_pac]
#            print(" i_square " +str(i_square), file=sys.stderr)
#            print(" Adj 0  " +str(adjacents_squares[i_square][0]), file=sys.stderr)
#            print(" Adj 2  " +str(adjacents_squares[i_square][2]), file=sys.stderr)
#            print(" Adj 4  " +str(adjacents_squares[i_square][4]), file=sys.stderr)
#            print(" Adj 6  " +str(adjacents_squares[i_square][6]), file=sys.stderr)

            adjs = liste_adj[i_pac]
            if adjacents_squares[i_square][0] == square_pellet :
                found = True #Haut
                adjs[0].append(square_pellet)
 #               print(" Attribué au pac " +str(i_pac) +" Pellet "+str(square_pellet), file=sys.stderr)
            elif adjacents_squares[i_square][2] == square_pellet:
                found = True # Droite
                adjs[1].append(square_pellet)
  #              print(" Attribué au pac " +str(i_pac) +" Pellet "+str(square_pellet), file=sys.stderr)
            elif adjacents_squares[i_square][4] == square_pellet:
                found = True # Bas
                adjs[2].append(square_pellet)
   #             print(" Attribué au pac " +str(i_pac) +" Pellet "+str(square_pellet), file=sys.stderr)
            elif adjacents_squares[i_square][6] == square_pellet:
                found = True  # Gauche 
                adjs[3].append(square_pellet)
                #print(" Attribué au pac " +str(i_pac) +" Pellet "+str(square_pellet), file=sys.stderr)                           
            else :
                i_pac = i_pac + 1
            
#recherche ds les  listes des cases visibles par chaque pac
        print("  aux  pacs ", file=sys.stderr)
        i_pac = 0
        while found == -1 and i_pac < nb_mine_pacs :
            i_square = pacs_square_index[i_pac]
            for i_adj in range(4):
                adjs = liste_adj[i_square][i_adj]
                i_adj_dir = 0 #Pour parcourir la liste des pellets visible ds une direction
                while not found and i_adj_dir < len(adjs):
                    i_square = adjs[i_adj_dir]
                    if adjacents_squares[i_square][0] == square_pellet :
                        found = True
                        adjs.append(liste_1[i_pellet])
                        #print("  2 Attribué au pac " +str(i_pac) +" Pellet "+str(i_pellet), file=sys.stderr)                           
                    else :
                        i_adj_dir = i_adj_dir + 1
 
    print(" Fin décodage pellets ", file=sys.stderr)
    ...
    #Ce n'est pas le premier tour    m
        for n_pac in range(nb_mine_pacs) :
            if pacs_x[n_pac] == old_x[n_pac]  and pacs_y[n_pac] == old_y[n_pac] :
                print(" Pac immobile " +str(n_pac), file=sys.stderr)
...
###################################################################################
    ...
    print(" Affectation sur bigs pellets ", file=sys.stderr)
    ...
    print(" Affectation sur pellets visibles ", file=sys.stderr)
    if not all_affected(pacs_affectation):
    ...
    #n_pac = 0
    print(" Afectation si rien en vue ", file=sys.stderr)
    for n_pac in   range(nb_mine_pacs) :
...
# Envoi ds la ligne de commmande d'un ordre par pac :  move de déblocage ou move normal  ou speed
    ...
    commandes = ""
    print(" Envoi des commandes ", file=sys.stderr)
            
    while n_pac < nb_mine_pacs :
        if  positions[n_pac] != '':
            commandes = commandes + "MOVE "+mine_pacs[n_pac]+" "+positions[n_pac]
            print(" Move "+str(n_pac) +" "+ positions[n_pac], file=sys.stderr)
...
#Sauvgardes des infos des pacs avant  application des commandes du tour
    ...
    print(" Fin du tour ", file=sys.stderr)
