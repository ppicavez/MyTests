...
def compute_dist_pac_pellet(x1,y1,x2,y2,width) :
    dist = 0
...
#  x = x prec y = y prec et ordre MOVE
    ...
    while not all_affected(pacs_affectation) :
        ...
        if not all_affected(big_affectations) :
            ...
            d_mini = 1000
            index_mini = 0
            for n_pellet in len(liste_10) :
                if not big_affectations[n_pellet]:
                    index_pellet = int(liste_10[n_pellet])
                    d = compute_dist(pacs_xn_paci],pacs_y[n_pac],
        ...
        #Recherche dans les pastilles visibles les plus proches
            ...
            y = int(liste_y[index_mini])
            pacs_affectation[n_pac] = True
            small_affetation[n_pellet] = True
        else :
            #Rem :
            ...
            d_mini = 1000
            index_mini = 0
            for n_pellet in len(squares_not_explored) :
