...
def find_adjacentes(x,y,sx,sy,width) :
    ...
    result.append(index_square(x - 1,y + 1,sx,sy,width))
    result.append(index_square(x -1 ,y - 1,sx,sy,width))
 #   print("Adj de " +str(x)+ " "+str(y), file=sys.stderr)
 #   if result[0] != -1 :
 #       print("Adjs 0 :" +str(result[0])+ " X "+str(sx[result[0]])+ " Y "+str(sy[result[0]]), file=sys.stderr)
 #   if result[1] != -1 :
 #       print("Adjs 1 :" +str(result[1])+ " X "+str(sx[result[1]])+ " Y "+str(sy[result[1]]), file=sys.stderr)
 #   if result[2] != -1 :
 #       print("Adjs 2 :" +str(result[2])+ " X "+str(sx[result[2]])+ " Y "+str(sy[result[2]]), file=sys.stderr)
 #   if result[3] != -1 :
 #       print("Adjs 3 :" +str(result[3])+ " X "+str(sx[result[3]])+ " Y "+str(sy[result[3]]), file=sys.stderr)
...
for n_square in range(nb_squares):
    ...
    #   if adjacents_squares[n_square][j] > 0 :
    #       print(" Case = " + str(j) + "  coord = "+str(squares_x[adjacents_squares[n_square][j]])+ ' '+
    #       "-"+str(squares_y[adjacents_squares[n_square][j]]), file=sys.stderr)   
...
for n_square in range(nb_squares):
    ...
    for i_dir in range(4):
        ...
        l_dir = 0
        current_square = adjacents_squares[n_square][i_dir]
        while  current_square!= -1 :
            #print("Passage 333 ", file=sys.stderr)
            l_dir = l_dir + 1
            current_square = adjacents_squares[current_square][i_dir]


        directions.append(l_dir)
        #print("d =  " +str(l_dir), file=sys.stderr)
    adjacents_segment_length.append(directions)
    if squares_x[n_square] == 9999 :
        print("Square No  "+str(n_square)+ " coord = "+str(squares_x[n_square])+ ' '+
                 "-"+str(squares_y[n_square]), file=sys.stderr)
        for j in range(4) :
           print(" Dir  = " + str(j) + "  d = "+str(adjacents_segment_length[n_square][j]), file=sys.stderr)   
...
while True:
    ...
    for i in range(visible_pac_count):
        ...
        if mine :
            ...
            if len(old_x) > 0 :
                V_x.append(x - old_x[n_pac])
                V_y.append(y - old_y[n_pac])
            n_pac = n_pac + 1
        else :
...
#Création liste des pellets à 10 points et liste des pellets à 10 points
...
#            print("Small "+ str(i)+"  = "+str(x)+ " "+str(y), file=sys.stderr)    
    ...
    print(" Nb  SmallPellet "+str(nb_small_pellet), file=sys.stderr)
    print(" Nb  Pellet "+str(len(liste_x)), file=sys.stderr)
    

    # Attribuer les pellets aux pacs
    #for n_pac in range(nb_mine_pacs):
    #    print(" No pac "+str(n_pac), file=sys.stderr)
    #    for i_dir in range(4) :
    #        l = adjacents_segment_length[pacs_square_index[n_pac]][i_dir]
    #        print(" Direction "+str(i_dir)+" l = "+str(l), file=sys.stderr)
    #print(" -------------------------------------------", file=sys.stderr)
        ...
        for i_dir in range(4) :
            ...
            if l > 0 :
                ...
                while i_pellet < nb_small_pellet :
                    index_pellet = liste_1[i_pellet]
                    square_pellet = liste_no_square[index_pellet] 
                    #print(" Pellet "+str(i_pellet), file=sys.stderr)
                    #print(" Index Pellet "+str(index_pellet), file=sys.stderr)
                    #print(" square Pellet "+str(square_pellet), file=sys.stderr)
                    #print(" x Pellet "+str(liste_x[index_pellet]), file=sys.stderr)
                    #print(" y Pellet "+str(liste_y[index_pellet]), file=sys.stderr)
#                    print(" Pac No "+str(n_pac), file=sys.stderr)
#                    print(" x Pac "+str(pacs_x[n_pac]), file=sys.stderr)
#                    print(" y Pac "+str(pacs_y[n_pac]), file=sys.stderr)
                        
                    if pacs_x[n_pac] == liste_x[index_pellet] or pacs_y[n_pac] == liste_y[index_pellet] :
                        # Le pac et le pellet sont sur une meme ligne ou une meme colonne
                        
                        found = False
                        #print(" Pelet et pac allignés ", file=sys.stderr)
                       # print(" i_dir = "+str(i_dir) +" X = "+str(pacs_x[n_pac])+" "+str(liste_x[index_pellet]), file=sys.stderr)
                        #print( "Y ="+str(pacs_y[n_pac])+" "+str(liste_y[index_pellet]), file=sys.stderr)
                        if i_dir == 0  and  pacs_x[n_pac] == liste_x[index_pellet]: #Haut
                            #print(" Dir 0 ", file=sys.stderr)
                            # On recherche si le pellet est ds le segment haut du pac
                            i_segment = 0
                            print(" l =" +str(l), file=sys.stderr)
                            while i_segment < l :
                                y = pacs_y[n_pac] - 1 - i_segment
                                if y == liste_y[index_pellet] :
                                    liste_adj[n_pac][0].append(square_pellet)
                                    #print(" En haut du pac", file=sys.stderr)
                                    nb_afect = nb_afect + 1
                                i_segment = i_segment + 1

                        if i_dir == 1  and pacs_y[n_pac] == liste_y[index_pellet]: #Haut
                            #print(" Dir 1 ", file=sys.stderr)
                            # On recherche si le pellet est ds le segment droite du pac
                            i_segment = 0
                            while   i_segment < l :
                                x = pacs_x[n_pac] + 1 +i_segment
                                if x == liste_x[index_pellet] :
                                    liste_adj[n_pac][1].append(square_pellet)
                                    #print(" A droite du pac", file=sys.stderr)
                                    nb_afect = nb_afect + 1
                                
                                i_segment = i_segment + 1
                        if i_dir == 2  and pacs_x[n_pac] == liste_x[index_pellet]: #Haut
                            #print(" Dir 2 ", file=sys.stderr)
                            ...
                            while i_segment < l :
                                ...
                                if y == liste_y[index_pellet] :
                                    ...
                                    #print(" En bas du pac", file=sys.stderr)
                                    nb_afect = nb_afect + 1
                                i_segment = i_segment + 1
                        elif i_dir == 3  and  pacs_y[n_pac] == liste_y[index_pellet]: #Haut
                            #print(" Dir 3 ", file=sys.stderr)
                            1# On recherche si le pellet est ds le segment gauche du pac
                            i_segment = 0
                            while not found and i_segment < l :
                                x = pacs_x[n_pac] - 1 - i_segment
                                if x == liste_x[index_pellet] :
                                    liste_adj[n_pac][3].append(square_pellet)
                                    #print(" A gauche du pac", file=sys.stderr)
                                    nb_afect = nb_afect + 1
                                i_segment = i_segment + 1
                    i_pellet = i_pellet + 1
                    #print(" Pellet suivant", file=sys.stderr)
    #if not found :
        #print(" Pellet non affecté", file=sys.stderr)
 ...
 # la liste des cases non explored squares_not_explored
  #  print(" Boucle ", file=sys.stderr)
    for n_pac in range(nb_mine_pacs) :
        #print(" Pac " + str(n_pac), file=sys.stderr)
        for i_dir in range(4) :
            #print("Direct " +str(i_dir), file=sys.stderr)
        
            l = adjacents_segment_length[pacs_square_index[n_pac]][i_dir]
            if l > 0 :
                liste_view =liste_adj[n_pac][i_dir] #liste des pellets vus parle pac ds une direction
                print(" l = "+str(l)+" nb vus "+str(len(liste_view)), file=sys.stderr)
                if len(liste_view) < l :
                    # Il y a des cases vides ds le segment vue
                    # Ces cases sont occupées par un pac ennemy
                    # ou on été vidées à un tour précédent
                    # il faut les retirer de la liste des cases non explorée

                    x = pacs_x[n_pac]
                    y = pacs_y[n_pac]
                    for i_segment in range(l) :
                        if i_dir == 0:
                            y = pacs_y[n_pac] - 1 - i_segment
                        elif i_dir ==1:
                            x = pacs_x[n_pac] + 1 + i_segment    
                        ...
                        segment_square = index_square(x,y,squares_x,squares_y,width)
                        if  segment_square in squares_not_explored and not (segment_square in liste_view) :        
                                squares_not_explored.remove(segment_square)
                                nb_squares_not_explored = nb_squares_not_explored - 1
...
#Analyse des colisions entre mes pacs
    ...
    #Ce n'est pas le premier tour    m
        print(" Colision " + str(len(V_x)), file=sys.stderr)
        for n_pac in range(nb_mine_pacs) :
            print(" Vx = " +str(V_x[n_pac]) + " Vy = "+str(V_y[n_pac]), file=sys.stderr)    
            if  V_x[n_pac] == 0  and V_y[n_pac] == 0:
        ...
        if nb_blocked > 1:
            ...
            found = False
            n_square = 0
            while not found and n_square < 4 :
...
#Sauvgardes des infos des pacs avant  application des commandes du tour
    ...
    while i < old_nb_mine :
        ...
        if len(V_x) > 0 :
            old_V_x.append(V_x[i])
            old_V_y.append(V_y[i])
        i = i+ 1        
