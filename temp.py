...
def compute_dist(x1,y1,x2,y2,width,s_x,s_y,  d_attaque) :
    dist = 0
    #print("Coord : "+str(x1)+ " "+str(y1) + " "+str(x2)+ " "+str(y2), file=sys.stderr)

# On privilegie les pelets qui sont en line de mire 
# Meme x ou meme y
# Attention au repliement de l'axe des x. 
# On peut faire le tour pour joindre les 2 points
...
#            print("Big "+ str(i)+"  = "+str(x)+ " "+str(y), file=sys.stderr)
 ...
 #           print("Small "+ str(i)+"  = "+str(x)+ " "+str(y), file=sys.stderr)    
    ...
    for i_pellet in range(nb_small_pellet) :
        found = -1    
        i_pac = 0
        ...
        while found == -1 and i_pac < nb_mine_pacs :
            ...
            if adjacents_squares[i_square][0] == liste_1[i_pellet] :
                found = 0 #Haut
                adjs[0].append(liste_1[i_pellet])
            elif adjacents_squares[i_square][2] == liste_1[i_pellet]:
                found = 1 # Droite
                adjs[1].append(liste_1[i_pellet])
            elif adjacents_squares[i_square][4] == liste_1[i_pellet]:
                found = 2 # Bas
                adjs[2].append(liste_1[i_pellet])
            elif adjacents_squares[i_square][6] == liste_1[i_pellet]:
                found = 3  # Gauche 
                adjs[3].append(liste_1[i_pellet])                           
            else :
                i_pac = i_pac + 1
...
#recherche ds les  listes des cases visibles par chaque pac 
        while found == -1 and i_pac < nb_mine_pacs :
            for i_adj in range(4):
                ...
                while found == -1 and i_adj_dir < len(adjs):
        ...
        for i_pac in range(nb_mine_pacs):
            ...
            if not pacs_affectation[i_pac] :
                ...
                for i_dir in range(4):
                    ...
                    print("Test len" +str(n_len) , file=sys.stderr)
                ...
                if nb_maxi_seen_pellet > 0:
                    ...
                    x =  liste_x[liste_adj[i_pac][dir_max][0]]
                    y =  liste_y[liste_adj[i_pac][dir_max][0]]
