    ...
    if len(liste_cdes) > 0 : # au 1er round il n'y a pas de cde precedente envoyee
        ...
        while i< len(mine_pacs) :
            ...
            if index_ok and (liste_cdes[j] == 'M') and (old_x[j] ==pacs_x[i]) and (old_y[j] == pacs_y[i]) :
                ...
                # case (x  +-1, y +- 1) sans pac propres ni pac ennemey

                #Recherche index de la case de blocage
                ok = False
                m = 0
                while not ok and m < len(pacs_x) :
                    if  (squares_x[m] == pacs_x[i]) and (squares_y[m] == pacs_y[i]):
                        ok = True
                    else :
                        m = m + 1    

                directions = adj_squares[m]

                # occupée par un des mes autres pacs ?
                d = 0
                ok = False
                while not ok and d < 8 : # Les 8 directions adjacentes
                    index_case_adj = directions[d]
                    if  (index_case_adj != -1) :
                        # Il y a une case adjacente dans la direction d
                        # Si  elle n'est pas occupée par un autre de mes pacs
                        #c'est une case de fuite
                       
                        ok = True
                    else :
                        d = d + 1
                # occupées par un pac ennemy ? (todo)
