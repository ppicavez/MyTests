...
##################################################################################################
# Ici commence la gestion des pacs
# - Analyse des blocages : MOVE
# - Speed des pacs
# - Chasser   les big pellet
# - Chasser les pacs ennemy ds le champ de vision
# - Chasser les pellets ds le champ de vision
# - chasser les autres Pellets non visible ou degrouper mes pacs
...
#  x = x prec y = y prec et ordre MOVE
    ...
    print("  PASSAGE -", file=sys.stderr)
    if len(liste_cdes) > 0 : # au 1er round il n'y a pas de blocage
        i = 0
        pacs_bloques = []
        nb_pacs_bloques = 0
        for i in range(nb_mine_pacs) :
        ...
        #Recherche des pac blocages entre mes pacs
        i = 0
        while i < nb_mine_pacs :
        ...
        if nb_pacs_bloques > 0 :
            ...
            while  i < nb_mine_pacs :
                ...
                if found :
                    ...
                    print(" MOVE = "+positions[i]+" MOVE  "+positions[j], file=sys.stderr)
                    n = 1/0
                i = i + 1

# Calculer les positions de deblocage
                # Envoyer pac i et j dans des directions différentes
                ...
                #    print("  DEBLOCAGE --", file=sys.stderr)



# Gestion du speed des pacs
    print("  PASSAGE -", file=sys.stderr)
...
# Envoi ds la ligne de commmande d'un ordre par pac :  move de déblocage ou move normal  ou speed
    ...
    commandes = ""
    while n_pacs < nb_mine_pacs :
        if positions[n_pacs] != "" :
            commandes = commandes + "MOVE "+mine_pacs[n_pacs]+" "+positions[n_pacs]
            liste_cdes.append('M')
        elif speeded[n_pacs] :
            commandes = commandes + "SPEED "+mine_pacs[n_pacs]
            liste_cdes.append('SP') 
        elif  transformed[n_pacs] != '' :
            commandes = commandes + "SWITCH "+mine_pacs[n_pacs] + transformed[n_pacs]
            liste_cdes.append('SW')
                
        if n_pacs < nb_mine_pacs - 1 :
            commandes= commandes +"|"
        in_pacs = n_pacs + 1
...
#Sauvgardes des infos des pacs 
    old_nb_mine = nb_mine_pacs
    n_pacs  = 0
    old_id = []
    old_x  = []
    old_y  = []  
    for n_pacs in rnge (old_nb_mine) :
        old_id.append(mine_pacs[n_pacs])
        old_x.append(pacs_x[n_pacs])
        old_y.append(pacs_y[n_pacs])       
